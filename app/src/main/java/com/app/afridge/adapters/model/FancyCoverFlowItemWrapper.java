 ...
 * <p/>
 * It is responsible for applying additional effects to each coverflow item, that can only be
 * applied at view level
 * (e.g. color saturation).
 * <p/>
 * This is a ViewGroup by intention to enable child views in layouts to stay interactive (like
 * buttons) though
 * transformed.
 * <p/>
 * Since this class is only used within the FancyCoverFlowAdapter it doesn't need to check if there
 * are multiple
...
public class FancyCoverFlowItemWrapper extends ViewGroup {
    ...
    public void setReflectionEnabled(boolean hasReflection) {
        ...
        if (hasReflection != this.isReflectionEnabled) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
                ...
                this.setLayerType(
                        hasReflection ? View.LAYER_TYPE_SOFTWARE : View.LAYER_TYPE_HARDWARE, null);
            }
    ...
    protected void dispatchDraw(@NonNull Canvas canvas) {
        ...
        canvas.drawBitmap(this.wrappedViewBitmap, (this.getWidth() - childView.getWidth()) / 2, 0,
                paint);
    }
    ...
    private void remeasureChildren() {
        ...
        if (child != null) {
            ...
            this.originalScaledownFactor = this.isReflectionEnabled ?
                    (originalChildHeight * (1 - this.imageReflectionRatio) - reflectionGap)
    ...
    private void createReflectedImages() {
        ...
        final Bitmap invertedBitmap = Bitmap
                .createBitmap(this.wrappedViewBitmap, 0, invertedBitmapSourceTop, width,
                        invertedHeight, matrix, true);

        this.wrappedViewDrawingCanvas
                .drawBitmap(invertedBitmap, 0, scaledDownHeight + reflectionGap, null);

        final Paint paint = new Paint();
        final LinearGradient shader = new LinearGradient(0,
                height * imageReflectionRatio + reflectionGap, 0, height, 0x70ffffff, 0x00ffffff,
