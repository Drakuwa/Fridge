        ...
        return new Filter() {
            ...
            private List<GalleryItem> getFilteredResults(CharSequence constraint) {
                ...
                if (constraint.toString()
                        .equalsIgnoreCase(inflater.getContext().getString(R.string.hint_all))) {
                    return allItems;
                }
                ...
                for (GalleryItem item : allItems) {
                    if (item.getId() == TAKE_PHOTO || item.getId() == CHOOSE_PHOTO || item
                            .getBucketName().equalsIgnoreCase(constraint.toString())) {
