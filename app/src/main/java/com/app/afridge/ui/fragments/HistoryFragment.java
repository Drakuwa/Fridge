 ...
 */
public class HistoryFragment extends Fragment implements LoaderManager.LoaderCallbacks<List<HistoryItem>>,
  ...
  public void onResume() {
    ...
    ((MainActivity) getActivity()).setActionBarTitle(application.getString(R.string.history).toLowerCase(Locale.ENGLISH));
      ...
      builder.setMessage(application.getString(R.string.clear_history_confirmation))
              .setPositiveButton(application.getString(R.string.delete), new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog, int id) {
                  ...
                  List<FridgeItem> items = new Select().from(FridgeItem.class).where("status = ?", true).execute();
                  ...
                  new Delete().from(FridgeItem.class).where("status = ?", true).execute();
                }
              })
                ...
                public void onClick(DialogInterface dialog, int which) {
                  ...
                  switch (which) {
                    case 0:
                      ...
                      textFilter.setText(getString(R.string.today).toUpperCase(Locale.US));
                      break;
                    case 1:
                      // yesterday
                      ...
                      Log.d(Log.TAG, application.dateFormat.format(new Date(cal.getTimeInMillis())));
                      restartLoader(cal.getTimeInMillis() / 1000);
                      textFilter.setText(getString(R.string.yesterday).toUpperCase(Locale.US));
                      break;
                    case 2:
  ...
  public void onDateSet(DatePicker view, int year, int monthOfYear, int dayOfMonth) {
    ...
    Log.d(Log.TAG, application.dateFormat.format(new Date(calendar.getTimeInMillis())));
    restartLoader(calendar.getTimeInMillis() / 1000);
    textFilter.setText(application.dateFormat.format(new Date(calendar.getTimeInMillis())).toUpperCase(Locale.US));
  }
