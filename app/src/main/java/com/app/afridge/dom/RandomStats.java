...
public class RandomStats {
    ...
    public void generateList(boolean refreshStats) {
        ...
        if (stats.size() == 0 || refreshStats) {
            ...
            int randomSpinCount = SharedPrefStore.load(context)
                    .getInt(SharedPrefStore.Pref.STAT_RANDOM_SPIN);
            int openFridgeCount = SharedPrefStore.load(context)
                    .getInt(SharedPrefStore.Pref.STAT_FRIDGE_OPEN);
            int cheeseItemsCount = new Select().from(FridgeItem.class).where("type = ?", "3")
                    .and("status = ?", false).count();
            int meatItemsCount = new Select().from(FridgeItem.class).where("type = ?", "4")
                    .and("status = ?", false).count();
            int leftoversItemsCount = new Select().from(FridgeItem.class).where("type = ?", "10")
                    .and("status = ?", false).count();
            int drinksItemsCount = new Select().from(FridgeItem.class).where("type = ?", "16")
                    .and("status = ?", false).count();
            int cakeItemsCount = new Select().from(FridgeItem.class).where("type = ?", "20")
                    .and("status = ?", false).count();
            ...
            int expiredItemsCount = new Select().from(FridgeItem.class)
                    ...
                    .count();
            int thrownAwayItems = new Select().from(FridgeItem.class).where("status = ?", true)
                    .count();
            ...
            stats.add(new Stat("Expired items you should consider getting rid of:",
                    expiredItemsCount));
