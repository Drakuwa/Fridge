      ...
      do {
        if (items.getInt(6) != 1) {
          ...
          if (items.getString(7).length() > 0) {
            ...
            try {
              fridgeItem.setExpirationDate(sdf.parse(items.getString(7)).getTime() / 1000);
            }
          ...
          if (items.getString(2).equalsIgnoreCase("ajvar")) {
            ...
            fridgeItem.setType(String.valueOf(ItemType.valueOf(items.getString(2).toUpperCase(Locale.ENGLISH)).ordinal()));
          }
          String quantityType = items.getString(4);
          if (quantityType.length() > 0) {
            if (quantityType.equalsIgnoreCase("Kilogram[s]") || quantityType.equalsIgnoreCase("Pound[s]")) {
              fridgeItem.setTypeOfQuantity(0);
            }
            else if (quantityType.equalsIgnoreCase("Gram[s]") || quantityType.equalsIgnoreCase("Ounce[s]")) {
              fridgeItem.setTypeOfQuantity(1);
            }
            else if (quantityType.equalsIgnoreCase("Liter[s]") || quantityType.equalsIgnoreCase("Gallon[s]")) {
