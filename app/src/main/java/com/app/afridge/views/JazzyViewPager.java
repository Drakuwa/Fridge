...
public class JazzyViewPager extends ViewPager {
    ...
    public JazzyViewPager(Context context, AttributeSet attrs) {
        ...
        int effect = ta.getInt(R.styleable.JazzyViewPager_jazzystyle, 0);
    ...
    private void animateRotate(View left, View right, float positionOffset, boolean up) {
        ...
        if (mState != State.IDLE) {
            if (left != null) {
                ...
                mTrans = (up ? -1 : 1) * (float) (getMeasuredHeight() - getMeasuredHeight() * Math
                        .cos(mRot * Math.PI / 180.0f));
                ViewHelper.setPivotX(left, left.getMeasuredWidth() * 0.5f);
        ...
        if (mState != State.IDLE) {
            if (left != null) {
                ...
                if (positionOffset < -1) {
                    ...
                    float scaleFactor = Math
                            .max(ZOOM_OUT_IN_MIN_SCALE, 1 - Math.abs(positionOffset));
            ...
            if (right != null) {
                ...
                if (positionOffset < -1) {
                    ...
                    float scaleFactor = Math
                            .max(ZOOM_OUT_IN_MIN_SCALE, 1 - Math.abs(positionOffset - 1));
                    float vertMargin = pageHeight * (1 - scaleFactor) / 2;
                    float horzMargin = pageWidth * (1 - scaleFactor) / 2;
