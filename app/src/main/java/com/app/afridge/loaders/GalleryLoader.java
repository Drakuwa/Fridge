...
public class GalleryLoader extends AsyncTaskLoader<List<GalleryItem>> {
    ...
    private List<GalleryItem> cursorToListAdapter(Cursor data) {
        ...
        while (data.moveToNext()) {
            ...
            galleryItem.setBucketId(
                    data.getString(data.getColumnIndex(MediaStore.Images.Media.BUCKET_ID)));
            galleryItem.setBucketName(data.getString(
                    data.getColumnIndex(MediaStore.Images.Media.BUCKET_DISPLAY_NAME)));
            galleryItem.setImageUri(
                    Uri.withAppendedPath(MediaStore.Images.Media.EXTERNAL_CONTENT_URI,
                            Long.toString(galleryItem.getId())));
